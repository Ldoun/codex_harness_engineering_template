...
description: Check and repair consistency across docs/product.md, docs/engineering.md, docs/ux.md, docs/plan.md, docs/spec-index.md when present, and the active spec referenced by docs/plan.md. Use when the user asks for a document consistency review or repair, or when creating or revising a spec and you need to align product, engineering, UX, plan, and spec documents.
---

# Doc Consistency Check

## Overview
Check the repo's canonical docs for role violations, contradictions, linkage drift, and incompleteness.
Start with findings, then repair only when the task context clearly calls for edits.
...
4. Read `docs/product.md`, `docs/engineering.md`, `docs/ux.md`, `docs/plan.md`, and the active spec if one is named and is not `docs/specs/_template.md`.
...
- Treat `docs/plan.md` as the sole source of the active spec and `docs/spec-index.md` as the visibility or index layer for all specs.
...
- Treat missing visual-input linkage as incompleteness when the active spec changes UI or declares visual requirements but lacks a source of truth or validation path.
- Do not invent missing product, UX, or architecture decisions. Surface them as open questions.
...
- Focus on five issue classes:
  - role violations
  - contradictions
  - linkage drift
  - incompleteness
  - missing visual-input or UX-contract linkage when the active spec changes UI
