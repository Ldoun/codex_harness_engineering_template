...
Use this skill for broad, eval-driven improvement work where Codex should repeatedly plan, implement, evaluate, and re-plan against a concrete rubric. This is not the default implementation workflow.
...
1. Read `AGENTS.md`, `docs/plan.md`, `docs/engineering.md`, and the active spec if one exists.
...
3. Rewrite the broad goal into an evaluation contract with:
   ...
   - an iteration budget
   - stop conditions
4. Keep `docs/plan.md` lean. Put the loop history, hypotheses, scores, and artifact references in the active iterative spec.
5. When useful, spawn read-only audit subagents for exploration or critique, then choose one milestone-sized hypothesis for the next pass.
6. Implement only one milestone-sized improvement at a time.
7. Run the evaluation contract, save artifacts, and compare against the previous best or baseline.
8. Update the iterative spec with the iteration result, evidence, score change, and next step.
9. Repeat until the stop conditions are met, the iteration budget is exhausted, or progress stalls.
10. When the loop is truly ready for normal closeout, set `Loop Status: ready-for-final-validation` and hand off to `$validate-active-spec`.
