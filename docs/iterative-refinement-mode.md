...
```text
$iterative-refinement-loop refine the mobile web UI for checkout.
Use a 3-iteration budget.
Focus the rubric on hierarchy, spacing, tap comfort, state clarity, and consistency.
Stop when hard gates pass and the rubric average is at least 4.2/5.
```
